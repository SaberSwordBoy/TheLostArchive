...
def index():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Index\n")
    return render_template('index.html')

@app.route('/about')
def about():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - About\n")
    return render_template('about.html')

@app.route('/people')
def cast():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - People\n")
    people = []
...
def person(name):
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Person/{name}\n")
    picture=False
    data = {'Name': "ERROR"}
    try: 
...
def behind_the_scenes():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Bts\n")
    return render_template('bts.html')

@app.route('/contact')
def contact():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Contact\n")
    return render_template('contact.html')

@app.route('/submit', methods=['GET', 'POST'])
def submit_user():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Submit\n")
...
def get_involved():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Getinvolved\n")
...
def sponsors():
    with open(ACCESSLOG, 'a') as f:
        f.write(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')} | {request.method}: {request.remote_addr} - Sponsors\n")
...
def get_picture(name):
    ...
    return send_file(f'/root/saberfilmsapp/static/images/{name}', attachment_filename='img.jpg')
...
def downloads(name):
    ...
    if 'pdf' in name:
        ...
        return send_file(f'/root/saberfilmsapp/downloads/{name}', attachment_filename='TLA_Image.jpg')
...
if __name__ == "__main__":
    app.run(host=HOST, 
            port=PORT, 
            ssl_context=('/root/saberfilmsapp/certs/cert.pem', '/root/saberfilmsapp/certs/key.pem'), 
            debug=True)
